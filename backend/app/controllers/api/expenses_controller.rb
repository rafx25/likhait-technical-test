class Api::ExpensesController < ApplicationController
  def index
    # Order by the expense date (most recent first) so newly added expenses
    # appear at the top of the list. created_at is used only as a tie-breaker
    # for expenses that share the same date.
    ...
    if params[:year].present? && params[:month].present?
      ...
      # Filter by the expense date rather than the record's creation timestamp.
